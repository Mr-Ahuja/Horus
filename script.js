...
function begin(){ landing?.classList.add('hidden'); startStars(false); if(!roomHash) roomHash=location.hash.substring(1); if(!roomTitle){ roomTitle = randomTitle(); } if(els.roomTitle) els.roomTitle.textContent = roomTitle; const wait=document.getElementById('waitMsg'); if(wait) wait.style.display='block'; roomName='observable-'+roomHash; drone = new ScaleDrone('yiS12Ts5RdNhebyM'); attach(); routeToSpeaker(); }
...
function randomTitle(){ const words=['Aurora','Nebula','Orion','Lyra','Vega','Cygnus','Draco','Phoenix','Sirius','Altair','Lumen','Comet','Quasar','Zenith','Nova']; return words[Math.floor(Math.random()*words.length)]; }
